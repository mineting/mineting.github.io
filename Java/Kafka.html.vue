<template><div><div align='center'><font size='70'><b>Kafka</b></font></div>
<blockquote>
<p>【Refer】</p>
<p>文档：</p>
<p>官网地址：<a href="https://kafka.apache.org/" target="_blank" rel="noopener noreferrer">https://kafka.apache.org/<ExternalLinkIcon/></a></p>
<p>视频：</p>
<p><a href="https://www.bilibili.com/video/BV1vr4y1677k/?spm_id_from=333.337.search-card.all.click" target="_blank" rel="noopener noreferrer">https://www.bilibili.com/video/BV1vr4y1677k/?spm_id_from=333.337.search-card.all.click<ExternalLinkIcon/></a></p>
</blockquote>
<h1 id="kafka-入门" tabindex="-1"><a class="header-anchor" href="#kafka-入门" aria-hidden="true">#</a> kafka 入门</h1>
<h2 id="kafka-概述" tabindex="-1"><a class="header-anchor" href="#kafka-概述" aria-hidden="true">#</a> Kafka 概述</h2>
<p>对于网站，可以通过前端埋点的形式上传用户操作日志，这些数据通过 Flume 采集，采集速度为 100M/S，Hadoop 上传速度为 100M/S，则可以正常消费，如果 Flume 采集速度过快为 200 M/S，则 Hadoop 消费不了，则需要中间件进行辅助，先将数据导入到中间件中，再由 Hadoop 后续慢慢消费。</p>
<h3 id="定义" tabindex="-1"><a class="header-anchor" href="#定义" aria-hidden="true">#</a> 定义</h3>
<ul>
<li>
<p>Kafka 传统定义：<strong>Kafka 是一个分布式的基于发布/订阅模式的消息队列（Message Queue）</strong>，主要应用于大数据实时处理领域。</p>
<p>发布/订阅：消息的发布者不会将消息直接发送给特定的订阅者，而是将发布的消息 分为不同的类别，订阅者只接收感兴趣的消息。</p>
</li>
<li>
<p>Kafka 最新定义 ： <strong>Kafka是一个开源的分布式事件流平台 （Event Streaming Platform）</strong>，被数千家公司用于高性能数据管道、流分析、数据集成和关键任务应用。</p>
</li>
</ul>
<h3 id="消息队列" tabindex="-1"><a class="header-anchor" href="#消息队列" aria-hidden="true">#</a> 消息队列</h3>
<p>消息队列，是消息也是队列；目前企业中常见的消息队列产品主要有 Kafka、ActiveMQ 、RabbitMQ 、 RocketMQ 等。</p>
<p>在大数据场景主要采用 Kafka 作为消息队列。在 JavaEE 开发中主要采用 ActiveMQ、 RabbitMQ、RocketMQ</p>
<h4 id="消息队列的应用场景" tabindex="-1"><a class="header-anchor" href="#消息队列的应用场景" aria-hidden="true">#</a> 消息队列的应用场景</h4>
<p>主要应用场景包括：缓存/消峰、解耦和异步通信</p>
<ul>
<li>
<p>缓冲/消峰：有助于控制和优化数据流经过系统的速度，解决生产消息和消费消息的处理速度不一致的情况。</p>
<figure><img src="@source/Java/Kafka/Kafka消息队列-削峰.png" alt="Kafka消息队列-削峰.png" tabindex="0" loading="lazy"><figcaption>Kafka消息队列-削峰.png</figcaption></figure>
</li>
<li>
<p>解耦：允许你独立的扩展或修改两边的处理过程，只要确保它们遵守同样的接口约束。</p>
<figure><img src="@source/Java/Kafka/Kafka消息队列-解耦.png" alt="Kafka消息队列-解耦.png" tabindex="0" loading="lazy"><figcaption>Kafka消息队列-解耦.png</figcaption></figure>
</li>
<li>
<p>异步通信：允许用户把一个消息放入队列，但并不立即处理它，然后在需要的时候再去处理它们。</p>
<figure><img src="@source/Java/Kafka/Kafka消息队列-异步.png" alt="Kafka消息队列-异步.png" tabindex="0" loading="lazy"><figcaption>Kafka消息队列-异步.png</figcaption></figure>
</li>
</ul>
<h4 id="消息队列的两种模式" tabindex="-1"><a class="header-anchor" href="#消息队列的两种模式" aria-hidden="true">#</a> 消息队列的两种模式</h4>
<p><strong>点对点模式</strong></p>
<ul>
<li>消费者主动拉取数据，消息收到后清除消息</li>
</ul>
<figure><img src="@source/Java/Kafka/Kakfa消息队列-点对点模式.png" alt="Kakfa消息队列-点对点模式.png" tabindex="0" loading="lazy"><figcaption>Kakfa消息队列-点对点模式.png</figcaption></figure>
...
<li>
<p>相比较于点对点模式，可处理更复杂的模式</p>
<figure><img src="@source/Java/Kafka/Kafka消息队列-发布订阅模式.png" alt="Kafka消息队列-发布订阅模式.png" tabindex="0" loading="lazy"><figcaption>Kafka消息队列-发布订阅模式.png</figcaption></figure>
</li>
</ul>
<h3 id="kafka-基础架构" tabindex="-1"><a class="header-anchor" href="#kafka-基础架构" aria-hidden="true">#</a> Kafka 基础架构</h3>
<ol>
<li>为方便扩展，并提高吞吐量，一个 topic 分为多个 partition（防止生产者往一个 Topic 投放的数据太多，机器硬盘不够，因此采用分区，将数据分往不同的机器）</li>
<li>配合分区的设计，提出消费者组的概念（提高消费能力），组内每个消费者并行消费（一个分区的主题只能由一个消费者进行消费，方式多个消费者消费消费同一个数据）</li>
<li>为提高可用性，为每个 partition 增加若干副本，类似 NameNode HA（防止一个 broker 挂掉，引入副本 Follower，副本有机会称为 Leader，消费者只针对 Leader 进行消费）</li>
<li>在 Zookeeper 中记录 Leader，和分区副本，Kafka2.8.0 以后也可以自行配置而不采用 Zookeeper</li>
</ol>
<figure><img src="@source/Java/Kafka/Kafka基本架构.png" alt="Kafka基本架构.png" tabindex="0" loading="lazy"><figcaption>Kafka基本架构.png</figcaption></figure>
<blockquote>
<p><strong>各个组件说明：</strong></p>
<ol>
<li>
<p><strong>Producer：消息生产者，就是向 Kafka broker 发消息的客户端</strong></p>
</li>
<li>
<p><strong>Consumer：消息消费者，向 Kafka broker 取消息的客户端</strong></p>
</li>
<li>
<p><strong>Consumer Group（CG）：消费者组，由多个 consumer 组成。消费者组内每个消费者负责消费不同分区的数据，一个分区只能由一个组内消费者消费；消费者组之间互不影响。所有的消费者都属于某个消费者组，即消费者组是逻辑上的一个订阅者。</strong></p>
</li>
<li>
<p><strong>Broker：一台 Kafka 服务器就是一个 broker。一个集群由多个 broker 组成。一个 broker 可以容纳多个 topic</strong></p>
</li>
<li>
<p><strong>Topic：可以理解为一个队列，生产者和消费者面向的都是一个 topic</strong></p>
</li>
<li>
<p><strong>Partition：为了实现扩展性，一个非常大的 topic 可以分布到多个 broker（即服 务器）上，一个 topic 可以分为多个 partition，每个 partition 是一个有序的队列。</strong></p>
</li>
<li>
<p><strong>Replica：副本，一个 topic 的每个分区都有若干个副本，一个 Leader 和若干个 Follower。</strong></p>
</li>
<li>
<p><strong>Leader：每个分区多个副本的 “主”，生产者发送数据的对象，以及消费者消费数据的对象都是 Leader。</strong></p>
</li>
<li>
<p><strong>Follower：每个分区多个副本中的 “从”，实时从 Leader 中同步数据，保持和 Leader 数据的同步。Leader 发生故障时，某个 Follower 会成为新的 Leader。</strong></p>
</li>
</ol>
</blockquote>
<h2 id="kafka-入门示例" tabindex="-1"><a class="header-anchor" href="#kafka-入门示例" aria-hidden="true">#</a> Kafka 入门示例</h2>
<p>Kafka 下载地址：<a href="https://kafka.apache.org/downloads" target="_blank" rel="noopener noreferrer">https://kafka.apache.org/downloads<ExternalLinkIcon/></a></p>
<p>以 <code v-pre>kafka_2.12-3.0.0.tgz</code> 为例，Kafka 由 Scala 和 Java 两种语言编写，Producer 和 Consumer 由 Java 编写，Broker 由 Scala 编写，Scala 版文为 2.12，Kafka 为 3.0</p>
<h3 id="预先准备" tabindex="-1"><a class="header-anchor" href="#预先准备" aria-hidden="true">#</a> 预先准备</h3>
<h4 id="配置-java-环境" tabindex="-1"><a class="header-anchor" href="#配置-java-环境" aria-hidden="true">#</a> 配置 Java 环境</h4>
<ul>
<li>
<p>卸载自带的 OpenJDK（可选）</p>
</li>
<li>
<p>配置 Java 环境变量</p>
...
<h4 id="配置集群免密登录" tabindex="-1"><a class="header-anchor" href="#配置集群免密登录" aria-hidden="true">#</a> 配置集群免密登录</h4>
<ul>
<li>
<p>配置集群 ip 映射</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># vim /etc/hosts</span>
...
<p>生成秘钥</p>
<p>会在 ~ 目录下生成 .ssh 目录</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost ~<span class="token punctuation">]</span><span class="token comment"># ssh-keygen -t rsa</span>
...
<li>
<p>分发秘钥</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost ~<span class="token punctuation">]</span><span class="token comment"># ssh-copy-id 192.168.100.100</span>
...
<h4 id="分发脚本-xsync-sh" tabindex="-1"><a class="header-anchor" href="#分发脚本-xsync-sh" aria-hidden="true">#</a> 分发脚本 <a href="http://xsync.sh" target="_blank" rel="noopener noreferrer">xsync.sh<ExternalLinkIcon/></a></h4>
<blockquote>
<ul>
...
<li>软链接（可选）：ln -s /opt/script/xsync.sh /usr/local/bin/</li>
</ul>
</blockquote>
...
<span class="token keyword">done</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="查看-java-进程-jpsall-sh" tabindex="-1"><a class="header-anchor" href="#查看-java-进程-jpsall-sh" aria-hidden="true">#</a> 查看 Java 进程  <a href="http://jpsall.sh" target="_blank" rel="noopener noreferrer">jpsall.sh<ExternalLinkIcon/></a></h4>
<p>创建查看进程脚本，并创建软链接：<code v-pre>ln -s /opt/script/jpsall.sh /usr/local/bin/</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token shebang important">#!/bin/bash</span>
<span class="token keyword">for</span> <span class="token for-or-select variable">host</span> <span class="token keyword">in</span> <span class="token number">192.168</span>.100.100 <span class="token number">192.168</span>.100.110 <span class="token number">192.168</span>.100.120
<span class="token keyword">do</span>
 <span class="token builtin class-name">echo</span> <span class="token operator">==</span><span class="token operator">==</span><span class="token operator">==</span><span class="token operator">==</span><span class="token variable">$host</span><span class="token operator">==</span><span class="token operator">==</span><span class="token operator">==</span><span class="token operator">==</span>
...
<span class="token keyword">done</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="安装步骤" tabindex="-1"><a class="header-anchor" href="#安装步骤" aria-hidden="true">#</a> 安装步骤</h3>
<p>以 <code v-pre>apache-zookeeper-3.5.7-bin.tar.gz</code> 、<code v-pre>kafka_2.12-3.0.0.tgz</code> 为例。</p>
<h4 id="安装-zookeeper" tabindex="-1"><a class="header-anchor" href="#安装-zookeeper" aria-hidden="true">#</a> 安装 Zookeeper</h4>
<h5 id="单机版" tabindex="-1"><a class="header-anchor" href="#单机版" aria-hidden="true">#</a> 单机版</h5>
<p>Zookeeper 地址：<a href="https://zookeeper.apache.org/releases.html" target="_blank" rel="noopener noreferrer">https://zookeeper.apache.org/releases.html<ExternalLinkIcon/></a></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>tickTime = 2000：通信心跳时间，Zookeeper 服务器与客户端心跳时间，单位毫秒</p>
<p>initLimit = 10：LF 初始通信时限，Leader 和 Follower初始连接时能容忍的最多心跳数**（tickTime的数量）**</p>
<p>syncLimit = 5：LF 同步通信时限，Leader 和 Follower之间通信时间如果超过 syncLimit * tickTime，Leader 认为 Follwer 死掉，从服务器列表中删除 Follwer。</p>
<p>dataDir：保存 Zookeeper 中的数据，默认的 tmp目录，容易被 Linux 系统定期删除，所以一般不用默认的 tmp 目录。</p>
...
<h5 id="集群版" tabindex="-1"><a class="header-anchor" href="#集群版" aria-hidden="true">#</a> 集群版</h5>
<ol>
<li>
...
<li>
<p>通过 <a href="http://xsync.sh" target="_blank" rel="noopener noreferrer">xsync.sh<ExternalLinkIcon/></a> 分发配置文件，并修改文件 myid  中的编号</p>
</li>
<li>
<p>创建 Zookeeper 集群启动脚本 <code v-pre>zk.sh</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token shebang important">#!/bin/bash</span>
...
<li>
<p>启动 Zookeeper 集群：</p>
<p>通过 <code v-pre>zk.sh</code> 启动集群：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost shell<span class="token punctuation">]</span><span class="token comment"># zk.sh start</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>报错解决：</strong></p>
<ul>
<li>
<p>报错一：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 注意报错</span>
...
Using config: /opt/zookeeper/zookeeper-3.5.7/bin/../conf/zoo.cfg<br>
Starting zookeeper ... FAILED TO START<br>
[root@localhost zookeeper-3.5.7]#</p>
<h1 id="如果报错-error-java-home-is-not-set-and-java-could-not-be-found-in-path" tabindex="-1"><a class="header-anchor" href="#如果报错-error-java-home-is-not-set-and-java-could-not-be-found-in-path" aria-hidden="true">#</a> 如果报错：Error: JAVA_HOME is not set and java could not be found in PATH.</h1>
<h1 id="单机启动没有问题-但集群启动报错" tabindex="-1"><a class="header-anchor" href="#单机启动没有问题-但集群启动报错" aria-hidden="true">#</a> 单机启动没有问题，但集群启动报错</h1>
<h1 id="解决-在-zookeeper-bin-zkenv-sh-头部手动添加-java-home-opt-java-jdk1-8-0-131" tabindex="-1"><a class="header-anchor" href="#解决-在-zookeeper-bin-zkenv-sh-头部手动添加-java-home-opt-java-jdk1-8-0-131" aria-hidden="true">#</a> 解决：在 zookeeper/bin/zkEnv.sh 头部手动添加 JAVA_HOME=/opt/java/jdk1.8.0_131</h1>
<h1 id="修改完成后-重新分发" tabindex="-1"><a class="header-anchor" href="#修改完成后-重新分发" aria-hidden="true">#</a> 修改完成后，重新分发</h1>
...
<h4 id="安装-kafka" tabindex="-1"><a class="header-anchor" href="#安装-kafka" aria-hidden="true">#</a> 安装 Kafka</h4>
<p>官方下载地址：<a href="http://kafka.apache.org/downloads.html" target="_blank" rel="noopener noreferrer">http://kafka.apache.org/downloads.html<ExternalLinkIcon/></a></p>
<ul>
<li>
<p>集群主机：<code v-pre>192.168.100.100 、192.168.100.110 、192.168.100.120</code></p>
</li>
<li>
<p>上传并解压到 /opt/kafka/kafka3.x</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># ll</span>
...
<p>主要修改：<code v-pre>broker.id</code> 、 <code v-pre>log.dirs</code> 、<code v-pre>zookeeper.connect</code></p>
<ul>
<li>
<p><a href="http://broker.id" target="_blank" rel="noopener noreferrer">broker.id<ExternalLinkIcon/></a>：broker 的全局唯一编号，不能重复，只能是数字</p>
</li>
<li>
<p>log.dirs：kafka 运行日志（数据）存放的路径，路径不需要提前创建，kafka 自动创建，可以配置多个磁盘路径，路径与路径之间可以用 &quot;，&quot; 分隔</p>
</li>
<li>
<p>zookeeper.connect：配置连接 Zookeeper 集群地址（在 zk 根目录下创建 /kafka，方便管理）</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># broker 的全局唯一编号，不能重复，只能是数字</span>
...
<p>以及修改监听的主机：</p>
<ul>
<li>listeners 和 advertised.listeners</li>
<li>可以使用 <code v-pre>host.name=192.168.100.100</code> 代替以上</li>
</ul>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment">############################# Server Basics #############################</span>
...
<li>
<p>分发 Kafka 目录并修改 <a href="http://broker.id" target="_blank" rel="noopener noreferrer">broker.id<ExternalLinkIcon/></a></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka<span class="token punctuation">]</span><span class="token comment"># xsync.sh kafka3.x/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>修改 192.168.100.110 、192.168.100.120 的 <code v-pre>kafka3.x/config/server.propertis</code> 中的 <a href="http://broker.id" target="_blank" rel="noopener noreferrer">broker.id<ExternalLinkIcon/></a></p>
<p>192.168.100.110 ：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment">############################# Server Basics #############################</span>

<span class="token comment"># The id of the broker. This must be set to a unique integer for each broker.</span>
<span class="token assign-left variable">broker.id</span><span class="token operator">=</span><span class="token number">1</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>192.168.100.120：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment">############################# Server Basics #############################</span>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment">#  source /etc/profile</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>分发环境变量并刷新</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka<span class="token punctuation">]</span><span class="token comment">#  xsync /etc/profile</span>
...
<ul>
<li>
<p>第一步：启动 Zookeeper 集群</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment">#  zk.sh start</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>第二步：依次启动 <code v-pre>192.168.100.100 、192.168.100.110 、192.168.100.120</code> 的 Kafka</p>
<p>【启动命令：<code v-pre>kafka-server-start.sh -daemon /opt/kafka/kafka3.x/config/server.properties</code>】</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># /opt/kafka/kafka3.x/bin/kafka-server-start.sh -daemon /opt/kafka/kafka3.x/config/server.properties</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>查看进程</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># jpsall.sh</span>
...
<li>
<p>停止集群，依次停止 <code v-pre>192.168.100.100 、192.168.100.110 、192.168.100.120</code> 的 Kafka</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># /opt/kafka/kafka3.x/bin/kafka-server-stop.sh</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>注意：<strong>停止 Kafka 集群时，需要一定时间，一定要等 Kafka 所有节点进程全部停止后再停止 Zookeeper 集群</strong>。因为 Zookeeper 集群当中记录着 Kafka 集群相关信息，Zookeeper 集群一旦先停止， Kafka 集群就没有办法再获取停止进程的信息，只能手动杀死 Kafka 进程。</p>
</li>
<li>
<p>集群启停脚本 <code v-pre>kf.sh</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token shebang important">#! /bin/bash</span>
<span class="token keyword">case</span> <span class="token variable">$1</span> <span class="token keyword">in</span>
<span class="token string">"start"</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">;</span><span class="token punctuation">;</span>
<span class="token keyword">esac</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>**注意：**这里有个问题，单机执行 Kafka 启动命令可以运行，但是使用 <a href="http://kf.sh" target="_blank" rel="noopener noreferrer">kf.sh<ExternalLinkIcon/></a> 脚本执行不会启动 Kafka，且会报错：nohup: 无法运行命令&quot;java&quot;: 没有那个文件或目</p>
<p>录，需要修改 shell 脚本：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token shebang important">#! /bin/bash</span>
<span class="token keyword">case</span> <span class="token variable">$1</span> <span class="token keyword">in</span>
<span class="token string">"start"</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">;</span><span class="token punctuation">;</span>
<span class="token keyword">esac</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>设置权限并拷贝到 /usr/bin 目录下：（或建立软链接）</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost script<span class="token punctuation">]</span><span class="token comment"># chmod +x kf.sh</span>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># cat log/server.log</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>第一步：<strong>确定内存够用，可以修改，Kafka 默认 1G</strong></p>
<p>第二步：<strong>meta.properties 和 cluster_id 不一致</strong>，要么删除目录 <code v-pre>log.dirs=/opt/kafka/kafka3.x/datas</code>后重新启动（推荐），要么手动修改（不推荐）</p>
</li>
<li>
<p>启动报错：查看 logs/server.log 启动日志</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>org.apache.zookeeper.KeeperException<span class="token variable">$NodeExistsException</span><span class="token builtin class-name">:</span> KeeperErrorCode <span class="token operator">=</span> NodeExists
...
<h3 id="命令行操作" tabindex="-1"><a class="header-anchor" href="#命令行操作" aria-hidden="true">#</a> 命令行操作</h3>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># cd bin &amp;&amp; ls</span>
...
<li>生产者：<a href="http://kafka-console-producer.sh" target="_blank" rel="noopener noreferrer">kafka-console-producer.sh<ExternalLinkIcon/></a></li>
<li>消费者：<a href="http://kafka-console-consumer.sh" target="_blank" rel="noopener noreferrer">kafka-console-consumer.sh<ExternalLinkIcon/></a></li>
<li>主题（集群）：<a href="http://kafka-topics.sh" target="_blank" rel="noopener noreferrer">kafka-topics.sh<ExternalLinkIcon/></a></li>
</ul>
<h4 id="主题相关命令" tabindex="-1"><a class="header-anchor" href="#主题相关命令" aria-hidden="true">#</a> 主题相关命令</h4>
<p>查看主题操作的相关命令参数：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment">#主题命令</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>常用参数：</p>
<table>
<thead>
<tr>
<th>参数</th>
...
<li>
<p>查看所有主题</p>
<p>多个 broker 以逗号隔开**【建议使用】**</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># bin/kafka-topics.sh --bootstrap-server 192.168.100.100:9092 --list</span>
...
<li>
<p>创建主题 first</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># bin/kafka-topics.sh --bootstrap-server 192.168.100.100:9092 --create --partitions 1 --replication-factor 3 --topic first</span>
Created topic first.
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>查看主题详情</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># bin/kafka-topics.sh --bootstrap-server 192.168.100.100:9092 --describe --topic first</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Partition: 0【表示分配了一个分区，下标从 0 开始】</p>
<p>Leader: 2【表示 Leader 在下标为 2 的 broker 上】</p>
</li>
<li>
<p>增加主题分区数量，只能增加不能减少</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># bin/kafka-topics.sh --bootstrap-server 192.168.100.100:9092 --alter --partitions 2 --topic first</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>重新查看</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># bin/kafka-topics.sh --bootstrap-server 192.168.100.100:9092 --describe --topic first  </span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>此时有两个分区。</p>
</li>
<li>
<p>删除主题</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># bin/kafka-topics.sh --bootstrap-server 192.168.100.100:9092 --delete --topic first</span>
<span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment">#</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div></li>
</ul>
<h4 id="生产者相关命令" tabindex="-1"><a class="header-anchor" href="#生产者相关命令" aria-hidden="true">#</a> 生产者相关命令</h4>
<p>生产者操作脚本：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost bin<span class="token punctuation">]</span><span class="token comment"># kafka-console-producer.sh</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>相关参数：</p>
<table>
<thead>
<tr>
<th>参数</th>
...
<h4 id="消费者相关命令" tabindex="-1"><a class="header-anchor" href="#消费者相关命令" aria-hidden="true">#</a> 消费者相关命令</h4>
<p>消费者操作脚本：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost bin<span class="token punctuation">]</span><span class="token comment"># kafka-console-consumer.sh</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>相关参数：</p>
<table>
<thead>
<tr>
...
<li>
<p>注意创建好分区：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost opt<span class="token punctuation">]</span><span class="token comment"># kafka-console-consumer.sh --bootstrap-server 192.168.100.110:9092 --topic first</span>
...
<li>
<p>生产者发送消息</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-console-producer.sh --bootstrap-server 192.168.100.100:9092 --topic first</span>
<span class="token operator">></span>demo
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>消费者消费消息【消费者只能消费生产者发送消息的主机数据？尽量写全所有的 IP】</p>
<p>默认只消费主题中新生产的数据，如果包含主题所有的历史数据，需要添加参数 <code v-pre>--from-begining</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-console-consumer.sh --bootstrap-server 192.168.100.100:9092 --topic first</span>
<span class="token builtin class-name">test</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div></li>
</ul>
<h2 id="kafka-生产者" tabindex="-1"><a class="header-anchor" href="#kafka-生产者" aria-hidden="true">#</a> kafka 生产者</h2>
<h3 id="生产者消息发送流程" tabindex="-1"><a class="header-anchor" href="#生产者消息发送流程" aria-hidden="true">#</a> 生产者消息发送流程</h3>
<h4 id="发送原理" tabindex="-1"><a class="header-anchor" href="#发送原理" aria-hidden="true">#</a> 发送原理</h4>
<figure><img src="@source/Java/Kafka/Kafka生产者发送流程.png" alt="Kafka生产者发送流程.png" tabindex="0" loading="lazy"><figcaption>Kafka生产者发送流程.png</figcaption></figure>
<blockquote>
<ol>
<li>
<p>生产者通过 Main 线程发送消息，创建了 Producer 对象，调用 Send() 方法发送数据，根据业务需求经过拦截器对消息进行处理（一般不做处理），再由自己的序列化器进行序列化（跨节点通信），再通过分区器 Partitioner 规定每个数据发往那个分区，一个分区创建一个队列，即发往一个缓存队列中（在内存中创建），队列大小默认 32M，每一批次大小默认为 16K，发送数据时会创建批次大小，每个批次会从<code v-pre>内存池</code>中取出内存，当批次中的数据发送到 Kafka 集群中时，会将批次释放至内存池中。</p>
</li>
<li>
<p>再由 Sender 线程主动拉取数据，拉取数据时需要满足两个条件：</p>
<ul>
<li><strong>batch.size：只有数据积累到 batch.size 之后，sender 才会发送数据。默认 16K</strong></li>
<li><strong><a href="http://linger.ms" target="_blank" rel="noopener noreferrer">linger.ms<ExternalLinkIcon/></a>：如果数据迟迟未达到 batch.size，sender 等待 <a href="http://linger.ms" target="_blank" rel="noopener noreferrer">linger.ms<ExternalLinkIcon/></a> 设置的时间到了之后就会发送数据。单位ms，默认值是 0ms，表示没有延迟。</strong></li>
</ul>
<p>发送数据到 broker 时，每个分区的数据，会以节点的形式发送，每一个节点会有一个队列，如果发送给 broker 的数据没有应答，则队列最多会缓存 5 个 Request。</p>
</li>
<li>
<p>Selector 负责建立 Socket 连接并发送数据，broker 对于发送的数据会进行 ACK 应答，其中：</p>
<ul>
...
<h4 id="重要参数列表" tabindex="-1"><a class="header-anchor" href="#重要参数列表" aria-hidden="true">#</a> 重要参数列表</h4>
<table>
<thead>
<tr>
<th>参数名称</th>
...
<td>生产者连接集群所需的 broker 地址清单 。 例如 192.168.100.100:9092,192.168.100.110:9092,192.168.100.120:9092，可以设置 1 个或者多个，中间用逗号隔开。注意这里并非需要所有的 broker 地址，因为生产者从给定的 broker 里查找到其他 broker 信息</td>
</tr>
<tr>
...
<tr>
<td><a href="http://linger.ms" target="_blank" rel="noopener noreferrer">linger.ms<ExternalLinkIcon/></a></td>
...
<tr>
<td><a href="http://retry.backoff.ms" target="_blank" rel="noopener noreferrer">retry.backoff.ms<ExternalLinkIcon/></a></td>
...
<h3 id="异步发送-api" tabindex="-1"><a class="header-anchor" href="#异步发送-api" aria-hidden="true">#</a> 异步发送 API</h3>
<h4 id="普通异步发送" tabindex="-1"><a class="header-anchor" href="#普通异步发送" aria-hidden="true">#</a> 普通异步发送</h4>
<p>生产者发送时的异步指的是，数据发送到 RecordAccumulator 缓冲区中，无需等待 Sender 线程发送数据等待结果返回，即可再次发送数据到 RecordAccumulator 中。</p>
<p>示例：创建 Kafka 生产者，采用异步的方式发送到 Kafka Broker</p>
<ul>
<li>
<p>创建工程 Kafka_GettingStart</p>
</li>
<li>
<p>引入依赖：</p>
...
<li>
<p>启动消费者进行测试：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-console-consumer.sh --bootstrap-server 192.168.100.110:9092 --topic first</span>
...
<h4 id="异步发送回调" tabindex="-1"><a class="header-anchor" href="#异步发送回调" aria-hidden="true">#</a> 异步发送回调</h4>
<ul>
<li>
<p>添加回调，此处的回调为生产者发送至 RecordAccumulator 后的回调。</p>
...
<div class="language-tex line-numbers-mode" data-ext="tex"><pre v-pre class="language-tex"><code>Topic: first, Partition: 0
Topic: first, Partition: 0
Topic: first, Partition: 0
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li>
</ul>
<h3 id="同步发送-api" tabindex="-1"><a class="header-anchor" href="#同步发送-api" aria-hidden="true">#</a> 同步发送 API</h3>
<p>生产者同步发送指的是，数据发送到 RecordAccumulator 缓冲区中，之后等待 Sender 线程发送数据到 broker ，等待结果返回之后，再次发送下一批数据。</p>
<ul>
<li>
<p>只需在异步发送的基础上，调用 get( ) 方法即可</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>example<span class="token punctuation">.</span>kafka<span class="token punctuation">.</span>producer</span><span class="token punctuation">;</span>
...
<h3 id="生产者分区" tabindex="-1"><a class="header-anchor" href="#生产者分区" aria-hidden="true">#</a> 生产者分区</h3>
<h4 id="分区优点" tabindex="-1"><a class="header-anchor" href="#分区优点" aria-hidden="true">#</a> 分区优点</h4>
<ul>
<li>
<p>便于<strong>合理使用存储资源</strong>，每个 Partition 在一个 Broker 上存储，可以把海量的数据按照分区切割成一 块一块数据存储在多台 Broker 上。</p>
<p>合理控制分区的任务，可以实现负载均衡的效果</p>
</li>
<li>
<p><strong>提高并行度</strong>，生产者可以以分区为单位发送数据；消费者可以以分区为单位进行消费数据</p>
</li>
</ul>
<img src="@source/Java/Kafka/Kafka分区优点.png" alt="Kafka分区优点.png" style="zoom:80%;" />
<h4 id="生产者发送消息的分区策略" tabindex="-1"><a class="header-anchor" href="#生产者发送消息的分区策略" aria-hidden="true">#</a> 生产者发送消息的分区策略</h4>
<p>分区策略由默认的分区器 DefaultPartitioner 决定。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ProducerRecord</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">K</span><span class="token punctuation">,</span> <span class="token class-name">V</span><span class="token punctuation">></span></span> <span class="token punctuation">{</span>
    ...
    <span class="token comment">// ......</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>注意：通过示例时，如果初始只有一个分区，则指定的分区不存在时，程序会阻塞，因此需要确保分区的数量不止一个，方便观察分区策略</strong></p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">CustomProducerCallbackPartition</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>示例二：没有指明 partition 但是 key 存在，会对 key 的 hash 值与 topic 的 partition 进行取余得到 partition 值</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token keyword">int</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> <span class="token number">3</span><span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        ...
        <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="自定义分区器" tabindex="-1"><a class="header-anchor" href="#自定义分区器" aria-hidden="true">#</a> 自定义分区器</h4>
<p>自定义分区器时，需要实现 Partitioner 接口，重写 partition 方法，使用自定义分区器时，需要在 Properties 中配置参数</p>
<p>MyPartitioner：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>example<span class="token punctuation">.</span>kafka<span class="token punctuation">.</span>producer</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在 Properties 中配置参数：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="生产者如何提高吞吐量" tabindex="-1"><a class="header-anchor" href="#生产者如何提高吞吐量" aria-hidden="true">#</a> 生产者如何提高吞吐量</h3>
<p>设置几个重要参数：</p>
<ol>
<li>batch.size：批次大小，默认 16K
<ul>
<li><strong>每次发送到 broker 的消息数据量大小</strong></li>
</ul>
</li>
<li><a href="http://linger.ms" target="_blank" rel="noopener noreferrer">linger.ms<ExternalLinkIcon/></a>：等待时间，生产中一般设置为 5 - 100ms
<ul>
<li><strong>默认 0s，只要有消息就会被发送到 broker，但是会影响效率，当 <a href="http://linger.ms" target="_blank" rel="noopener noreferrer">linger.ms<ExternalLinkIcon/></a> 设置较大时，消息有延迟。</strong></li>
</ul>
</li>
<li>compression.type：压缩 snappy
<ul>
<li><strong>对批次数据进行压缩可以提高传输量，默认 none，可配置值 gzip、snappy（常用）、 lz4 和 zstd</strong></li>
</ul>
</li>
<li>RecordAccumulator：缓冲区大小，修改为 64M
<ul>
<li><strong>当生产者异步发送消息时，如果 RecordAccumulator 过小，会造成消息堆积，生产者端会阻塞。</strong></li>
</ul>
</li>
</ol>
<p>示例：</p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">CustomProducerParameter</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="数据可靠性" tabindex="-1"><a class="header-anchor" href="#数据可靠性" aria-hidden="true">#</a> 数据可靠性</h3>
...
<ul>
<li>
<p>0：表示生产者发送过来的数据不需要等待数据落盘应答</p>
<p><strong>如果 Leader 收到数据后，尚且没有落盘，此时 Leader 挂掉，那么数据就会丢失。</strong></p>
<img src="@source/Java/Kafka/ACK应答级别0.png" alt="ACK应答级别.0png" style="zoom:67%;" />
</li>
<li>
<p>1：表示生产者发送过来的数据，Leader 收到并落盘应答</p>
<p><strong>应答完成后，还没有同步副本，此时 Leader 挂掉，从剩余的 Follower 中选出 Leader，新的 Leader 不会收到消息，因为生产者认为已经发送成功了，会导致数据丢失。</strong></p>
<img src="@source/Java/Kafka/ACK应答级别1.png" alt="ACK 应答级别1.png" style="zoom: 80%;" />
</li>
<li>
<p>-1（all）：默认值，表示生产者发送过来的数据，Leader 和 ISR （可以理解为保持同步的 Leader 和 Follower 集合）队列中的所有节点收到数据落盘后应答，-1 和 all 等价，如果 Leader 收到数据，所有 Follower 都开始同步数据， 但有一个 Follower，因为某种故障，迟迟不能与 Leader 进行同步，这样就会导致 Leader 不能应答，进而因为一个 Follower 挂掉导致整个集群瘫痪。</p>
<p><strong>因此 Leader 维护了一个动态的 in-sync replica set（ISR），意为和 Leader 保持同步的 Follower + Leader 集合（leader：0，isr:0,1,2）。如果 Follower 长时间未向Leader 发送通信请求或同步数据，则该 Follower 将被踢出 ISR。该时间阈值由 <a href="http://replica.lag.time.max.ms" target="_blank" rel="noopener noreferrer">replica.lag.time.max.ms<ExternalLinkIcon/></a> 参数设定，默认 30s。这样就不用等待长期联系不上或者已经故障的节点。</strong></p>
<img src="@source/Java/Kafka/ACK应答级别all.png" alt="ACK 应答级别all.png" style="zoom: 80%;" />
</li>
</ul>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">CustomProducerACK</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="数据去重" tabindex="-1"><a class="header-anchor" href="#数据去重" aria-hidden="true">#</a> 数据去重</h3>
<p>**数据重复：**当应答级别为 all ，并且 Leader 和所有 Follower 都落盘后，Leader 还未应答，便已经挂掉，此时重新选举出 Leader 后，生产者重新发送消息，便会造成数据的重复。</p>
<img src="@source/Java/Kafka/ACK应答级别为all时数据重复.png" alt="ACK应答级别为all时数据重复.png" style="zoom:67%;" />
<h4 id="数据传递语义" tabindex="-1"><a class="header-anchor" href="#数据传递语义" aria-hidden="true">#</a> 数据传递语义</h4>
<ul>
<li>
<p>至少一次（At Least Once）= ACK 级别设置为 -1 + 分区副本大于等于 2 + ISR 里应答的最小副本数量大于等于2 【数据不丢】</p>
</li>
<li>
<p>最多一次（At Most Once）= ACK 级别设置为 0 【数据不重】</p>
</li>
<li>
<p>总结： At Least Once 可以保证数据不丢失，但是不能保证数据不重复； At Most Once 可以保证数据不重复，但是不能保证数据不丢失。</p>
</li>
<li>
<p>精确一次（Exactly Once）：对于一些非常重要的信息，比如和钱相关的数据，要求数据既不能重复也不丢失。</p>
</li>
<li>
<p>Kafka 0.11 版本以后，引入了一项重大特性：<strong>幂等性和事务</strong></p>
</li>
</ul>
...
<p>重复数据的判断标准：具有【PID、Partition、SeqNumber】相同主键（key）的消息提交时，Broker 只会持久化一条。其中：</p>
<ul>
<li>PID 是 Kafka 每次重启都会分配一个新的</li>
<li>Partition 表示分区号</li>
<li>Sequence Number是单调自增的</li>
</ul>
<p><strong>所以幂等性只能保证的是在单分区单会话内不重复，一旦重启仍可能重复，如果要保证不重复，需要事务支持。</strong></p>
<p>如以下两个消息分别发送两个不同的分区。</p>
<figure><img src="@source/Java/Kafka/幂等性原理.png" alt="幂等性原理.png" tabindex="0" loading="lazy"><figcaption>幂等性原理.png</figcaption></figure>
<p>**使用幂等性：**开启参数 enable.idempotence 默认为 true，false 关闭</p>
<h4 id="生产者事务" tabindex="-1"><a class="header-anchor" href="#生产者事务" aria-hidden="true">#</a> 生产者事务</h4>
<p><strong>Kafka 事务原理：</strong></p>
<p><strong>注意：开启事务，必须开启幂等性</strong></p>
<figure><img src="@source/Java/Kafka/生产者事务原理.png" alt="生产者事务原理.png" tabindex="0" loading="lazy"><figcaption>生产者事务原理.png</figcaption></figure>
<p>说明：</p>
<blockquote>
<ol>
<li>每个 Broker 都有一个事务协调器，事务需要持久化到磁盘上，有一个存储事务的特殊主题 Topic，该 Topic 有 50 个分区，每个分区负责一部分事务，需要手动指定事务 <a href="http://Transactional.id" target="_blank" rel="noopener noreferrer">Transactional.id<ExternalLinkIcon/></a> ，根据 hashcode % 50 决定采用这 50 个分区中的哪一个分区</li>
</ol>
</blockquote>
<h4 id="相关-api" tabindex="-1"><a class="header-anchor" href="#相关-api" aria-hidden="true">#</a> 相关 API</h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>示例：通过事务保证消息不重复也不丢失</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>example<span class="token punctuation">.</span>kafka<span class="token punctuation">.</span>producer</span><span class="token punctuation">;</span>
...
<figure><img src="@source/Java/Kafka/单分区数据有序.png" alt="单分区数据有序.png" tabindex="0" loading="lazy"><figcaption>单分区数据有序.png</figcaption></figure>
<ul>
<li>
<p>kafka 在 1.x 版本之前保证数据单分区有序，条件如下：</p>
<p>max.in.flight.requests.per.connection = 1（不需要考虑是否开启幂等性）。</p>
</li>
<li>
<p>kafka 在 1.x 及以后版本保证数据单分区有序，条件如下：</p>
<ul>
<li><strong>未开启幂等性</strong>： max.in.flight.requests.per.connection 需要设置为 1</li>
<li><strong>开启幂等性</strong>： max.in.flight.requests.per.connection 需要设置小于等于 5
<ul>
...
<h3 id="数据有序" tabindex="-1"><a class="header-anchor" href="#数据有序" aria-hidden="true">#</a> 数据有序</h3>
<ul>
<li>对于单分区内保证数据有序</li>
<li>但多分区，分区与分区之间是无序的，如果要保证多分区内数据有序，则可以在消费端进行数据的重排序。</li>
</ul>
<img src="@source/Java/Kafka/数据有序.png" alt="数据有序.png" style="zoom:67%;" />
<h2 id="kafka-broker" tabindex="-1"><a class="header-anchor" href="#kafka-broker" aria-hidden="true">#</a> Kafka Broker</h2>
<h3 id="kafka-broker-工作流程" tabindex="-1"><a class="header-anchor" href="#kafka-broker-工作流程" aria-hidden="true">#</a> Kafka Broker 工作流程</h3>
<h4 id="zookeeper-存储的-kafka-信息" tabindex="-1"><a class="header-anchor" href="#zookeeper-存储的-kafka-信息" aria-hidden="true">#</a> Zookeeper 存储的 Kafka 信息</h4>
<p>在 Zookeeper 中查看 kafka 的信息</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># zkCli.sh</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Zookeeper 中存储的 Kafka 信息：</p>
<p>![Zookeeper 中存储的 Kafka 信息](Kafka/Zookeeper 中存储的 Kafka 信息.png)</p>
<h4 id="kafka-broker-整体工作流程" tabindex="-1"><a class="header-anchor" href="#kafka-broker-整体工作流程" aria-hidden="true">#</a> Kafka Broker 整体工作流程</h4>
<p>![Kafka Broker 整体工作流程](Kafka/Kafka Broker 整体工作流程.png)</p>
<p>说明：</p>
<blockquote>
<ol>
<li>Broker 在 Zookeeper 中注册：<code v-pre>/brokers/ids/ [0,1,2]</code></li>
<li>每个 Broker 上都有一个 Controller，谁先注册进 Zookeeper，谁就是具有话语权的 Controller，则这个选举出来的 Controller 会监听 brokers 节点的变化，进行辅助选举</li>
<li>之后进行正式的选举，选举规则：在 ISR 中存活为前提，按照 AR（Kakfa 分区中所有副本的统称）中排列在前面的优先，例如 ISR【1, 0, 2】，AR【1, 0, 2】，则 Leader 会轮询从 1, 0, 2 中产生</li>
<li>此时由 Controller 将节点的信息上传至 Zookeeper，其他的 Controller 从 Zookeeper 中拉取同步相关信息（为了避免 Controller 挂掉）</li>
<li>Producer 发送消息给 Leader，同步给副本，文件以 Log （虚拟的）的形式存在，Log 由 Segment 实现（1G），保证读取数据，Segment 使用了 .log （数据）文件和 .index （索引）文件。</li>
<li>假设 Broker 中的 Leader 1挂掉，Controller 会监听到节点的变化，同时获取 ISR，重新选举，选举规则同上，选出 Leader 0 ，完成后更新 Leader 和 ISR</li>
</ol>
</blockquote>
<p>示例：演示 Kafka 节点的上下线</p>
<ol>
<li>
<p>关闭 192.168.100.120 broker</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>kafka-server-stop.sh
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>使用 Zookeeper 可视化工具 PrettyZoo 查看节点 brokers 的变化情况</p>
</li>
<li>
<p>重新启动 192.168.100.100 broker</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>kafka-server-start.sh <span class="token parameter variable">-daemon</span> /opt/kafka/kafka3.x/config/server.properties
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
</ol>
<h4 id="broker-重要参数" tabindex="-1"><a class="header-anchor" href="#broker-重要参数" aria-hidden="true">#</a> Broker 重要参数</h4>
<table>
<thead>
<tr>
<th>参数名称</th>
...
<tbody>
<tr>
<td><a href="http://replica.lag.time.max.ms" target="_blank" rel="noopener noreferrer">replica.lag.time.max.ms<ExternalLinkIcon/></a></td>
...
<tr>
<td>leader.imbalance.per.broker.percentag</td>
...
<tr>
<td><a href="http://log.retention.ms" target="_blank" rel="noopener noreferrer">log.retention.ms<ExternalLinkIcon/></a></td>
<td>Kafka 中数据保存的时间，毫秒级别，默认关闭。</td>
</tr>
<tr>
<td><a href="http://log.retention.check.interval.ms" target="_blank" rel="noopener noreferrer">log.retention.check.interval.ms<ExternalLinkIcon/></a></td>
<td>检查数据是否保存超时的间隔，默认是 5 分钟。</td>
</tr>
<tr>
<td>log.retention.bytes</td>
<td>默认等于-1，表示无穷大。超过设置的所有日志总 大小，删除最早的 segment。</td>
</tr>
<tr>
...
<tr>
<td><a href="http://log.flush.interval.ms" target="_blank" rel="noopener noreferrer">log.flush.interval.ms<ExternalLinkIcon/></a></td>
<td>每隔多久，刷数据到磁盘，默认是 null。一般不建 议修改，交给系统自己管理。</td>
</tr>
</tbody>
</table>
<h3 id="节点服役与退役" tabindex="-1"><a class="header-anchor" href="#节点服役与退役" aria-hidden="true">#</a> 节点服役与退役</h3>
<h4 id="服役新节点" tabindex="-1"><a class="header-anchor" href="#服役新节点" aria-hidden="true">#</a> 服役新节点</h4>
<p><strong>创建新节点</strong></p>
<ul>
<li>克隆一台新节点，修改 IP 以及主机名称 <code v-pre>vim /etc/hostname</code></li>
<li>删除 datas 和 logs 目录</li>
<li>修改 server.properties 的 <a href="http://broker.id" target="_blank" rel="noopener noreferrer">broker.id<ExternalLinkIcon/></a>，listeners，advertised.listeners</li>
<li>单独启动该节点</li>
</ul>
<p>示例：克隆主机为 192.168.100.130</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost ~<span class="token punctuation">]</span><span class="token comment"># cd /opt/kafka/kafka3.x/</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># zk.sh start</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>单独启动 192.168.100.130 的 kafka，查看主题 first ，发现 Replicas 等没有进行负载均衡。</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-server-start.sh -daemon ./config/server.properties</span>
...
<span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment">#</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>执行负载均衡操作</strong></p>
<ul>
<li>
<p>在 192.168.100.100 创建一个需要均衡的主题，如 first</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># vim topics-to-move.json</span>
<span class="token punctuation">{</span>
...
<li>
<p>生成一个负载均衡的计划，不满意可以重复执行</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-reassign-partitions.sh --bootstrap-server 192.168.100.100:9092 --topics-to-move-json-file topics-to-move.json --broker-list "0,1,2,3" --generate</span>
...
<p>将上述执行计划中的 <code v-pre>Proposed partition reassignment configuration</code>写入文件中</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># vim increase-replication-factor.json</span>
<span class="token punctuation">{</span><span class="token string">"version"</span>:1,<span class="token string">"partitions"</span>:<span class="token punctuation">[</span><span class="token punctuation">{</span><span class="token string">"topic"</span><span class="token builtin class-name">:</span><span class="token string">"first"</span>,<span class="token string">"partition"</span>:0,<span class="token string">"replicas"</span>:<span class="token punctuation">[</span><span class="token number">2,1</span>,3<span class="token punctuation">]</span>,<span class="token string">"log_dirs"</span>:<span class="token punctuation">[</span><span class="token string">"any"</span>,<span class="token string">"any"</span>,<span class="token string">"any"</span><span class="token punctuation">]</span><span class="token punctuation">}</span><span class="token punctuation">]</span><span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>执行副本存储计划</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-reassign-partitions.sh --bootstrap-server 192.168.100.100:9092 --reassignment-jn-file increase-replication-factor.json --execute</span>
Current partition replica assignment
...
<li>
<p>验证副本存储计划</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-reassign-partitions.sh --bootstrap-server 192.168.100.100:9092 --reassignment-json-file increase-replication-factor.json --verify</span>
...
<li>
<p>再次查看主题 first</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --describe --topic first</span>
Topic: first    TopicId: rTpcJQk3RmCjahRc8mAQiw PartitionCount: <span class="token number">1</span>       ReplicationFactor: <span class="token number">3</span>    Configs: <span class="token assign-left variable">segment.bytes</span><span class="token operator">=</span><span class="token number">1073741824</span>
...
<h4 id="退役旧节点" tabindex="-1"><a class="header-anchor" href="#退役旧节点" aria-hidden="true">#</a> 退役旧节点</h4>
<p>先按照退役一台节点，生成执行计划，然后按照服役时操作流程执行负载均衡，将数据需要迁移的数据均衡到其他 broker 上。</p>
<ul>
<li>
<p>在 192.168.100.100 创建一个需要均衡的主题，如 first</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># vim topics-to-move.json</span>
...
<li>
<p>创建执行计划</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-reassign-partitions.sh --bootstrap-server 192.168.100.100:9092 --topics-to-move-json-file topics-to-move.json --broker-list "0,1,2" --generate</span>
Current partition replica assignment
<span class="token punctuation">{</span><span class="token string">"version"</span>:1,<span class="token string">"partitions"</span>:<span class="token punctuation">[</span><span class="token punctuation">{</span><span class="token string">"topic"</span><span class="token builtin class-name">:</span><span class="token string">"first"</span>,<span class="token string">"partition"</span>:0,<span class="token string">"replicas"</span>:<span class="token punctuation">[</span><span class="token number">2,1</span>,3<span class="token punctuation">]</span>,<span class="token string">"log_dirs"</span>:<span class="token punctuation">[</span><span class="token string">"any"</span>,<span class="token string">"any"</span>,<span class="token string">"any"</span><span class="token punctuation">]</span><span class="token punctuation">}</span><span class="token punctuation">]</span><span class="token punctuation">}</span>
...
<li>
<p>执行副本存储计划</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-reassign-partitions.sh --bootstrap-server 192.168.100.100:9092 --reassignment-json-file increase-replication-factor.json --execute</span>
Current partition replica assignment
...
<li>
<p>192.168.100.130 执行停止命令</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-server-stop.sh</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
</ul>
<h3 id="kafka-副本" tabindex="-1"><a class="header-anchor" href="#kafka-副本" aria-hidden="true">#</a> Kafka 副本</h3>
<h4 id="副本基本信息" tabindex="-1"><a class="header-anchor" href="#副本基本信息" aria-hidden="true">#</a> 副本基本信息</h4>
<ol>
<li>
<p>Kafka 副本作用：提高数据可靠性。</p>
</li>
<li>
<p>Kafka 默认副本 1 个，生产环境一般配置为 2 个，保证数据可靠性；太多副本会增加磁盘存储空间，增加网络上数据传输，降低效率。</p>
</li>
<li>
<p>Kafka 中副本分为：Leader 和 Follower。Kafka 生产者只会把数据发往 Leader， 然后 Follower 找 Leader 进行同步数据。</p>
</li>
<li>
<p>Kafka 分区中的所有副本统称为 AR（Assigned Repllicas）。 AR = ISR + OSR</p>
<p>ISR，表示和 Leader 保持同步的 Follower 集合。如果 Follower 长时间未向 Leader 发送通信请求或同步数据，则该 Follower 将被踢出 ISR。该时间阈值由 <a href="http://replica.lag.time.max.ms" target="_blank" rel="noopener noreferrer">replica.lag.time.max.ms<ExternalLinkIcon/></a> 参数设定，默认 30s。Leader 发生故障之后，就会从 ISR 中选举新的 Leader。</p>
<p>OSR，表示 Follower 与 Leader 副本同步时，延迟过多的副本。</p>
</li>
</ol>
<h4 id="leader-选举流程" tabindex="-1"><a class="header-anchor" href="#leader-选举流程" aria-hidden="true">#</a> Leader 选举流程</h4>
<p>![Kafka Broker 整体工作流程](Kafka/Kafka Broker 整体工作流程.png)</p>
<p>说明：</p>
<blockquote>
<ol>
<li>Broker 在 Zookeeper 中注册：<code v-pre>/brokers/ids/ [0,1,2]</code></li>
<li>每个 Broker 上都有一个 Controller，谁先注册进 Zookeeper，谁就是具有话语权的 Controller，则这个选举出来的 Controller 会监听 brokers 节点的变化，进行辅助选举</li>
<li>之后进行正式的选举，选举规则：<strong>在 ISR 中存活为前提，按照 AR（Kakfa 分区中所有副本的统称）中排列在前面的优先</strong>，例如 ISR【1, 0, 2】，AR【1, 0, 2】，则 Leader 会轮询从 1, 0, 2 中产生</li>
<li>此时由 Controller 将节点的信息上传至 Zookeeper，其他的 Controller 从 Zookeeper 中拉取同步相关信息（为了避免 Controller 挂掉）</li>
<li>Producer 发送消息给 Leader，同步给副本，文件以 Log （虚拟的）的形式存在，Log 由 Segment 实现（1G），保证读取数据，Segment 使用了 .log （数据）文件和 .index （索引）文件。</li>
<li>假设 Broker 中的 Leader 1挂掉，Controller 会监听到节点的变化，同时获取 ISR，重新选举，选举规则同上，选出 Leader 0 ，完成后更新 Leader 和 ISR</li>
</ol>
</blockquote>
<p>示例：演示 Leader 的选举规则</p>
<p>准备 4 个 broker ，主题 example，分区 4 个，副本 4 个</p>
<ul>
<li>
<p>创建一个新的 topic，4 个分区，4 个副本，查看 Leader 分布情况</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --create --topic example --partitions 4 --replication-factor 4</span>
...
<li>
<p>停止掉 192.168.100.130 （broker.id=3）的 kafka 进程，并查看 Leader 分区情况</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --describe --topic example</span>
Topic: example  TopicId: Tslw9AFKRKCK0RgdO274pA PartitionCount: <span class="token number">4</span>       ReplicationFactor: <span class="token number">4</span>    Configs: <span class="token assign-left variable">segment.bytes</span><span class="token operator">=</span><span class="token number">1073741824</span>
...
<li>
<p>停止掉 192.168.100.120 （broker.id=2）的 kafka 进程，并查看 Leader 分区情况</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --describe --topic example</span>
Topic: example  TopicId: Tslw9AFKRKCK0RgdO274pA PartitionCount: <span class="token number">4</span>       ReplicationFactor: <span class="token number">4</span>    Configs: <span class="token assign-left variable">segment.bytes</span><span class="token operator">=</span><span class="token number">1073741824</span>
...
<li>
<p>启动 192.168.100.130 （broker.id=3）的 kafka 进程，并查看 Leader 分区情况</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --describe --topic example</span>
Topic: example  TopicId: Tslw9AFKRKCK0RgdO274pA PartitionCount: <span class="token number">4</span>       ReplicationFactor: <span class="token number">4</span>    Configs: <span class="token assign-left variable">segment.bytes</span><span class="token operator">=</span><span class="token number">1073741824</span>
...
<li>
<p>启动 192.168.100.120 （broker.id=2）的 kafka 进程，并查看 Leader 分区情况</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --describe --topic example</span>
Topic: example  TopicId: Tslw9AFKRKCK0RgdO274pA PartitionCount: <span class="token number">4</span>       ReplicationFactor: <span class="token number">4</span>    Configs: <span class="token assign-left variable">segment.bytes</span><span class="token operator">=</span><span class="token number">1073741824</span>
...
<li>
<p>停止掉 192.168.100.110 （broker.id=1）的 kafka 进程，并查看 Leader 分区情况【注意是从前往后（从下标为 0 开始）的顺序优先为 Leader】</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --describe --topic example</span>
Topic: example  TopicId: Tslw9AFKRKCK0RgdO274pA PartitionCount: <span class="token number">4</span>       ReplicationFactor: <span class="token number">4</span>    Configs: <span class="token assign-left variable">segment.bytes</span><span class="token operator">=</span><span class="token number">1073741824</span>
...
<h4 id="leader-和-follower-故障处理细节" tabindex="-1"><a class="header-anchor" href="#leader-和-follower-故障处理细节" aria-hidden="true">#</a> Leader 和 Follower 故障处理细节</h4>
<p><strong>Follower 故障</strong></p>
<ul>
<li>LEO （Log End Offset） : 每个副本的最后一个 offset，LEO 其实就是最新的 offset + 1</li>
<li>HW （High Watermark）: 所有副本中最小的 LEO。</li>
</ul>
<p>最初生产者和消费者只与 Leader 进行通信，Follower 从 Leader 拉取数据，HW 为所有副本中最小的 LEO</p>
<figure><img src="@source/Java/Kafka/Follower故障处理细节.png" alt="Follower故障处理细节" tabindex="0" loading="lazy"><figcaption>Follower故障处理细节</figcaption></figure>
<ol>
<li>
<p>Follower 故障</p>
<p>（1）Follower 发生故障后会被临时踢出 ISR</p>
<p>（2）这个期间 Leader 和 Follower 继续接收数据</p>
<p>（3）待该 Follower 恢复后，Follower 会读取本地磁盘记录的上次的 HW，并将 log 文件高于 HW 的部分截取掉，从 HW 开始向 Leader 进行同步</p>
<p>（4）等该 Follower 的 LEO 大于等于该 Partition 的 HW，即 Follower 追上 Leader 之后，就可以重新加入 ISR 了</p>
</li>
<li>
<p>Leader 故障</p>
<figure><img src="@source/Java/Kafka/Leader故障处理细节.png" alt="Leader故障处理细节" tabindex="0" loading="lazy"><figcaption>Leader故障处理细节</figcaption></figure>
</li>
</ol>
<h4 id="分区副本分配" tabindex="-1"><a class="header-anchor" href="#分区副本分配" aria-hidden="true">#</a> 分区副本分配</h4>
<p>如果 kafka 服务器只有 4 个节点，那么设置 kafka 的分区数大于服务器台数，在 kafka 底层如何分配存储副本？</p>
<p>示例：创建 16 分区，3 个副本</p>
<ol>
<li>
<p>创建一个新的 Topic，名称为 second</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --create --partitions 16 --replication-factor 3 --topic second</span>
Created topic second.
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>查看分区和副本情况</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost kafka3.x<span class="token punctuation">]</span><span class="token comment"># kafka-topics.sh --bootstrap-server=192.168.100.100:9092 --describe --topic second</span>
...
<h4 id="手动调整分区副本存储" tabindex="-1"><a class="header-anchor" href="#手动调整分区副本存储" aria-hidden="true">#</a> 手动调整分区副本存储</h4>
<h4 id="leader-partition-负载平衡" tabindex="-1"><a class="header-anchor" href="#leader-partition-负载平衡" aria-hidden="true">#</a> Leader Partition 负载平衡</h4>
<h4 id="增加副本因子" tabindex="-1"><a class="header-anchor" href="#增加副本因子" aria-hidden="true">#</a> 增加副本因子</h4>
<h3 id="文件存储" tabindex="-1"><a class="header-anchor" href="#文件存储" aria-hidden="true">#</a> 文件存储</h3>
<h4 id="文件存储机制" tabindex="-1"><a class="header-anchor" href="#文件存储机制" aria-hidden="true">#</a> 文件存储机制</h4>
<h4 id="文件清除策略" tabindex="-1"><a class="header-anchor" href="#文件清除策略" aria-hidden="true">#</a> 文件清除策略</h4>
<h3 id="高效读写数据" tabindex="-1"><a class="header-anchor" href="#高效读写数据" aria-hidden="true">#</a> 高效读写数据</h3>
<h2 id="kafka-消费者" tabindex="-1"><a class="header-anchor" href="#kafka-消费者" aria-hidden="true">#</a> Kafka 消费者</h2>
<h3 id="kafka-消费方式" tabindex="-1"><a class="header-anchor" href="#kafka-消费方式" aria-hidden="true">#</a> Kafka 消费方式</h3>
<ul>
<li>
<p>pull（拉）模式</p>
<p>Consumer 采用从 broker 中主动拉取数据， <strong>Kafka 采用这种方式</strong></p>
<p><strong>pull 模式不足之处是，如果 Kafka 没有数 据，消费者可能会陷入循环中，一直返回空数据。</strong></p>
</li>
<li>
<p>push（推）模式</p>
<p>Kafka 没有采用这种方式，因为由 broker 决定消息发送速率，很难适应所有消费者的消费速率。例如推送的速度是 50M/s， Consumer1、Consumer2 就来不及处理消息</p>
</li>
</ul>
<figure><img src="@source/Java/Kafka/Kafka消费方式.png" alt="Kafka消费方式" tabindex="0" loading="lazy"><figcaption>Kafka消费方式</figcaption></figure>
<h3 id="kafka-消费者工作流程" tabindex="-1"><a class="header-anchor" href="#kafka-消费者工作流程" aria-hidden="true">#</a> Kafka 消费者工作流程</h3>
<h4 id="消费者总体工作流程" tabindex="-1"><a class="header-anchor" href="#消费者总体工作流程" aria-hidden="true">#</a> 消费者总体工作流程</h4>
<figure><img src="@source/Java/Kafka/消费者总体工作流程.png" alt="消费者总体工作流程" tabindex="0" loading="lazy"><figcaption>消费者总体工作流程</figcaption></figure>
<p>说明：</p>
<ol>
<li>
<p>一个消费者可以消费多个分区的数据，多个消费者可以消费同一个分区的数据</p>
</li>
<li>
<p>每个分区的数据只能由消费者组中一个消费者消费，防止重复消费等问题</p>
</li>
<li>
<p>每个消费者的消费进度 offset 由消费者提交到系统主题保存，保存在一个主题中。 kafka 0.9 版本之前会将消费进度存放在 Zookeeper 中，会造成大量的网络交互</p>
</li>
</ol>
<h4 id="消费者组原理" tabindex="-1"><a class="header-anchor" href="#消费者组原理" aria-hidden="true">#</a> 消费者组原理</h4>
<p>Consumer Group（CG）：消费者组，由多个 Consumer 组成。形成一个消费者组的条件，是所有消费者的 groupid 相同。</p>
<ul>
<li>消费者组内每个消费者负责消费不同分区的数据，一个分区只能由一个组内消费者消费。</li>
<li>消费者组之间互不影响。所有的消费者都属于某个消费者组，即消费者组是逻辑上的一个订阅者。</li>
</ul>
<figure><img src="@source/Java/Kafka/消费者组原理.png" alt="消费者组原理" tabindex="0" loading="lazy"><figcaption>消费者组原理</figcaption></figure>
<p>说明：</p>
<ol>
<li>如果只有一个消费者，则由该消费者消费所有分区数据</li>
<li>如果消费者数量不小于分区数量，则</li>
<li>如果消费者数量等于分区数量，则一对一消费</li>
<li>如果向消费组中添加更多的消费者，超过主题分区数量，则有一部分消费者就会闲置，不会接收任何消息</li>
<li>消费者组之间互不影响。所有的消费者都属于某个消费者组，即消费者组是逻辑上的一个订阅者</li>
</ol>
<h4 id="消费者组初始化流程" tabindex="-1"><a class="header-anchor" href="#消费者组初始化流程" aria-hidden="true">#</a> 消费者组初始化流程</h4>
<p>coordinator：辅助实现消费者组的初始化和分区的分配。</p>
<p>coordinator 节点选择 = groupid 的 hashcode 值 % 50（ <code v-pre>__consumer_offsets的分区数量</code>） 例如： groupid 的 hashcode 值 = 1，1% 50 = 1，那么<code v-pre>__consumer_offsets </code>主题的1号分区，在哪个 broker 上，就选择这个节点的 coordinator 作为这个消费者组的老大。</p>
<p>消费者组下的所有的消费者提交 offset 的时候就往这个分区去提交 offset。</p>
<figure><img src="@source/Java/Kafka/消费者组初始化流程.png" alt="消费者组初始化流程" tabindex="0" loading="lazy"><figcaption>消费者组初始化流程</figcaption></figure>
<h2 id="kafka-eagle-监控" tabindex="-1"><a class="header-anchor" href="#kafka-eagle-监控" aria-hidden="true">#</a> Kafka-Eagle 监控</h2>
<p>Kafka-Eagle 框架可以监控 Kafka 集群的整体运行情况，在生产环境中经常使用</p>
<h3 id="mysql-环境准备" tabindex="-1"><a class="header-anchor" href="#mysql-环境准备" aria-hidden="true">#</a> MySQL 环境准备</h3>
<p>Kafka-Eagle 的安装依赖于 MySQL，MySQL 主要用来存储可视化展示的数据。</p>
<ul>
<li>
<p>下载并解压相应安装包</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># pwd</span>
/opt/mysql5.7
...
<li>
<p>如果是虚拟机：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># rpm -qa | grep -i -E mysql\|mariadb | xargs -n1 sudo rpm -e --nodeps</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>如果是阿里云服务器（安装的是 Linux 最小版）</p>
<p>卸载 MySQL 依赖：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># yum remove mysql-libs</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>下载编译相关依赖并安装：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># yum install libaio</span>
...
<li>
<p>安装 MySQL 依赖：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># rpm -ivh mysql-community-common-5.7.16-1.el7.x86_64.rpm</span>
...
<li>
<p>安装 MySQL-Client</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># rpm -ivh mysql-community-client-5.7.16-1.el7.x86_64.rpm</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>安装 MySQL-Server</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># rpm -ivh mysql-community-server-5.7.16-1.el7.x86_64.rpm</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
</ul>
</li>
<li>
<p>启动 MySQL</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># systemctl start mysqld</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>查看 MySQL 密码</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token punctuation">[</span>root@localhost mysql5.7<span class="token punctuation">]</span><span class="token comment"># cat /var/log/mysqld.log | grep password</span>
...
<li>
<p>配置密码</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 使用上一步查询到的密码（带单引号）登录 MySQL</span>
...
<li>
<p>设置 MySQL 远程连接</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mysql<span class="token operator">></span> <span class="token keyword">select</span> user,host from user<span class="token punctuation">;</span>
+-----------+-----------+
...
<li>
<p>刷新权限并退出</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mysql<span class="token operator">></span> flush privileges<span class="token punctuation">;</span>
...
<h2 id="kafka-kraft-模式" tabindex="-1"><a class="header-anchor" href="#kafka-kraft-模式" aria-hidden="true">#</a> Kafka-Kraft 模式</h2>
<h1 id="kafka-集成" tabindex="-1"><a class="header-anchor" href="#kafka-集成" aria-hidden="true">#</a> Kafka 集成</h1>
<h2 id="集成-flume" tabindex="-1"><a class="header-anchor" href="#集成-flume" aria-hidden="true">#</a> 集成 Flume</h2>
<h2 id="集成-flink" tabindex="-1"><a class="header-anchor" href="#集成-flink" aria-hidden="true">#</a> 集成 Flink</h2>
<h2 id="集成-springboot" tabindex="-1"><a class="header-anchor" href="#集成-springboot" aria-hidden="true">#</a> 集成 SpringBoot</h2>
<h2 id="集成-spark" tabindex="-1"><a class="header-anchor" href="#集成-spark" aria-hidden="true">#</a> 集成 Spark</h2>
<h1 id="源码解析" tabindex="-1"><a class="header-anchor" href="#源码解析" aria-hidden="true">#</a> 源码解析</h1>
<h2 id="环境准备" tabindex="-1"><a class="header-anchor" href="#环境准备" aria-hidden="true">#</a> 环境准备</h2>
<h2 id="生产者源码" tabindex="-1"><a class="header-anchor" href="#生产者源码" aria-hidden="true">#</a> 生产者源码</h2>
<h2 id="消费者源码" tabindex="-1"><a class="header-anchor" href="#消费者源码" aria-hidden="true">#</a> 消费者源码</h2>
<h2 id="服务器源码" tabindex="-1"><a class="header-anchor" href="#服务器源码" aria-hidden="true">#</a> 服务器源码</h2>
<h1 id="生产调优" tabindex="-1"><a class="header-anchor" href="#生产调优" aria-hidden="true">#</a> 生产调优</h1>
<h2 id="kafka-硬件配置选择" tabindex="-1"><a class="header-anchor" href="#kafka-硬件配置选择" aria-hidden="true">#</a> Kafka 硬件配置选择</h2>
<h2 id="kafka-生产者-1" tabindex="-1"><a class="header-anchor" href="#kafka-生产者-1" aria-hidden="true">#</a> Kafka 生产者</h2>
<h2 id="kafka-broker-1" tabindex="-1"><a class="header-anchor" href="#kafka-broker-1" aria-hidden="true">#</a> kafka Broker</h2>
<h2 id="kafka-消费者-1" tabindex="-1"><a class="header-anchor" href="#kafka-消费者-1" aria-hidden="true">#</a> Kafka 消费者</h2>
<h2 id="kafka-总体" tabindex="-1"><a class="header-anchor" href="#kafka-总体" aria-hidden="true">#</a> kafka 总体</h2>
</div></template>
